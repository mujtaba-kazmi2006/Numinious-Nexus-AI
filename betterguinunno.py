...
import predictor_module # Assuming this module exists and works as expected
import numpy as np
...
def handle_question(prompt):
    ...
    def prediction_thread_func():
        ...
        try:
            ...
            df = predictor_module.fetch_binance_ohlcv(symbol=detected_token, interval=detected_tf)
            if stop_flag: return
            if df is None or df.empty:
                error_msg = f"❌ Could not fetch data for {detected_token}. Please try again later.\n"
                chat_log.insert(tk.END, error_msg, "error")
                conversation_history.append({"role": "assistant", "content": error_msg}) # Add error to history
                return

            df = predictor_module.add_indicators(df)
            if stop_flag: return
            
            (X_train, X_test, y_train, y_test), latest_input = predictor_module.prepare_dataset(df)
            if stop_flag: return
            
            prediction, confidence, accuracy, reasoning = predictor_module.train_and_predict(X_train, X_test, y_train, y_test, latest_input)
            if stop_flag: return
            ...
            if not stop_flag:
                ...
                # Add the prediction result itself to the history so AI can refer to it
                conversation_history.append({
                    "role": "assistant", # Using assistant role to represent this factual output
                    "content": f"Prediction for {detected_token} ({detected_tf}): Direction is {prediction_str}. Confidence: {confidence*100:.1f}%. Model Accuracy: {accuracy*100:.1f}%. Reasoning: {reasoning}"
                })
                
                # Now, ask Nunno to explain the full conversation context, including the new prediction result
                # You might add a specific follow-up prompt for explanation if needed
                explanation_prompt_content = f"Based on the prediction results for {detected_token}, can you break this down simply for a beginner and explain what it means for my trading?"
                
                # Create a temporary list for the API call to add the explanation prompt
                messages_for_api = manage_history_length(list(conversation_history)) # Ensure we pass truncated history
            ...
            if not stop_flag:
                ...
                chat_log.insert(tk.END, error_msg, "error")
                print(f"Prediction Thread Error: {e}") # Log error for debugging
                conversation_history.append({"role": "assistant", "content": f"An error occurred during prediction: {e}"}) # Add error to history
        finally:
            enable_input_controls()
